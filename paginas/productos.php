...
switch($accion){
		...
		while($registro=$consulta->fetch_array()){
			...
			<tr>
				<td>
					...
					?>
					<img src="images/<?php echo $registroi['archivoImg']; ?>" width="50">
	...
	case 'ver':
		...
		while($registroi=$consultai->fetch_array()){
			?>
			<img src="images/<?php echo $registroi['archivoImg']; ?>" width="100">
			<?php
		}
	...
	case 'borrar':
		if($_SESSION['conectado']){
			...
			if($consulta=$conexion->query($sql)){
				...
				//Si se borra el producto, BORRO sus imagenes
				//Primero voy a borrar las imagenes FISICAMENTE del directorio
				$sqli="SELECT * FROM imagenes WHERE idProd=$id";
				$consultai=$conexion->query($sqli);
				while($registroi=$consultai->fetch_array()){
					unlink('images/'.$registroi['archivoImg']);
				}
				//Una vez borradas del DIRECTORIO, las borro de la tabla
		...
		if($_SESSION['conectado']){
			if(isset($_POST['enviar'])){
				...
				if($consulta=$conexion->query($sql)){
					...
					//EN ESTE PUNTO VAMOS A AGREGAR IMAGENES
					...
					for($i=0;$i<count($_FILES['imagenes']['name']);$i++){
						if(is_uploaded_file($_FILES['imagenes']['tmp_name'][$i])){
							...
							move_uploaded_file($_FILES['imagenes']['tmp_name'][$i], 'images/'.$nombreImagen);
					...
					//AQUI FINALIZAMOS AREGAR IMAGENES
				...
				<form action="index.php?p=productos.php&accion=modificar&numeroDePagina=<?php echo $numeroDePagina; ?>" method="post" enctype="multipart/form-data">
					...
					while($registroi=$consultai->fetch_array()){
						...
						<div style="display: inline-block; padding:5px; text-align: center;">
							<img src="images/<?php echo $registroi['archivoImg'] ?>" width="100"><br>
							<a href="index.php?p=productos.php&accion=modificar&id=<?php echo $id; ?>&numeroDePagina=<?php echo $numeroDePagina; ?>&idImagenBorrar=<?php echo $registroi['idImg']; ?> "title="Borrar Imágen" ><span style="color: red;" class="glyphicon glyphicon-trash"></span></a>
						</div>
						<?php
					}
					//FIN DEL AÑADIDO
